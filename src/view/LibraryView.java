package view;

import dto.Student;
import service.LibraryService;

import java.sql.SQLException;
import java.util.Scanner;

/**
 * 5.20
 * 사용자 인터페이스
 * view 에 구현
 * <p>
 * view
 * 화면만드는 역할
 */
public class LibraryView {

    private final LibraryService service = new LibraryService();
    private final Scanner scanner = new Scanner(System.in);

    private Integer currentStudentID = null;
    private String currentStudentName = null;

    //접근주체
    //private Student pricipalUser = null;

    public void start() {

        while (true) { //계속 돌아가는 구문
            System.out.println("===== 도서 관리 시스템 =====");

            //현재 id 값이 null이라면
            if (currentStudentID == null) {
                System.out.println("현재 로그아웃 생태입니다.");
            } else {
                System.out.println("현재 로그인 유지" + currentStudentName);
            }

            System.out.println("1.도서 추가");
            System.out.println("2.도서 목록");
            System.out.println("3.도서 검색");
            System.out.println("4.학생 등록");
            System.out.println("5.학생 목록");
            System.out.println("6.도서 대출");
            System.out.println("7.대출 현황");
            System.out.println("8.도서 반납");
            System.out.println("9.로그인");
            System.out.println("10.로그아웃");
            System.out.println("11.시스템 종료");
            System.out.print("선택: ");

            int choice;

            try {
                choice = scanner.nextInt();

                scanner.nextLine(); //버퍼비우기
            } catch (Exception e) {
                System.out.println("올바른 번호를 입력해주세요");
                scanner.nextLine();
                continue;
            }//try-catch

            try {
                switch (choice) {
                    case 1:
                        System.out.println("도서 추가");
                        break;
                    case 2:
                        System.out.println("");
                        break;
                    case 3:
                        System.out.println("");
                        break;
                    case 4:
                        System.out.println("");
                        break;
                    case 5:
                        System.out.println("");
                        break;
                    case 6:
                        borrowBook();
                        break;
                    case 7:
                        System.out.println("");
                        break;
                    case 8:
                        System.out.println("");
                        break;
                    case 9:
                        login();
                        break;
                    case 10:
                        logout();
                        break;
                    case 11:
                        System.out.println("시스템 종료");
                        scanner.close(); //자원 해제
                        return;
                    default:
                        System.out.println("잘못된 입력입니다");
                }//switch
            } catch (SQLException e) {
                System.out.println("오류: "+e.getMessage());
            } catch (Exception e) {
                System.out.println("전체 오류"+e.getMessage());
            }//try-catch
        }//while
    }//end of start

    //로그인 기능 만들기
    private void login() throws SQLException {

        if (currentStudentID != null) {
            System.out.println("이미 로그인된 변호입니다");
            return;
        }
        System.out.println("학번: ");
        String studentID = scanner.nextLine().trim();
        if (studentID.isEmpty()) {
            System.out.println("학번을 입력해주세요");
            return;
        }
        //학번을 입력받았다면
        //1.존재하는 학번인지 확인하기 위해 db 조회 (view=> service=> dao)
        Student studentDTO = service.authenticateStudent(studentID);

        //메서드 수행의 결과 (객체 존재 , null)
        if (studentDTO == null) {
            System.out.println("존재하지 않는 학번입니다");
        } else {
            currentStudentID = studentDTO.getId(); //int => Integer 자동변환
            currentStudentName = studentDTO.getName();
            System.out.println("로그인 성공: " + currentStudentName);
        }
    }//

    //로그아웃 기능 만들기
    private void logout() {

        if (currentStudentID == null) {
            System.out.println("이미 로그인 상태가 아닙니다");

        } else {
            currentStudentID = null;
            currentStudentName = null;
            System.out.println("로그아웃 완료");
        }
    }//

    //
    private void borrowBook() throws SQLException {

        //로그인이 먼저 돼야 student_id 값을 넘겨줄 수 있다.
        if (currentStudentID == null) {
            System.out.println("로그인 먼저 해주세요");
            return;
        }

        System.out.print("도서id: ");
        int bookID;

        try {
            bookID = scanner.nextInt();
            if (bookID <= 0) {
                System.out.println("유효한 도서id를 입력해주세요");
                return;
            }
        } catch (Exception e) {
            System.out.println("유효한 도서id를 입력해주세요");
            return;
        }
        scanner.nextLine(); //버퍼 비우기
        service.borrowBook(bookID,currentStudentID);
        System.out.println("대출완료");
    }//


    //메인함수
    public static void main(String[] args) {

        LibraryView libraryView = new LibraryView();
        libraryView.start();

    }//main

}//LibraryView
